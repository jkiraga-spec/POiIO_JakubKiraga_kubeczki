...
void TCup:: desired_vol_percentage(std::string name, double vol_percentage)
{
	...
	{
		volume_total=volume_total+volumes[i];
		if(name == substances[i].get_name())
		{
			water_volume = volumes[get_substance_id(name)];
		}	
	}
	if((volume_total>0) && (volume_total<cup_volume))
	{
		...
		//double add_water = (vol_percentage /100 * volume_total ) / (1 - vol_percentage / 100);
...
void TCup:: desired_mass_percentage(std::string name, double mass_percentage)
{
	...
	for (int i=0; i<count; i++)
	{
		...
		if(name == substances[i].get_name())
		{
			water_mass = volumes[get_substance_id(name)] * substances[get_substance_id(name)].get_ro() * 1000;
		}
	}
	...
	if((mass_total>0)&&(volume_total<cup_volume))
	{
		...
		{	
			double add_water = (mass_percentage /100 * mass_total - water_mass ) / ((1 - mass_percentage / 100) * substancje[get_substance_id(name)].get_ro() * 1000 );
			if(add_water > 0)
			{
